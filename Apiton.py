...
class RouterConfigGUI:
    def __init__(self, root):
        ...
        self.ospf_tab = ttk.Frame(self.notebook)
        ...
        self.notebook.add(self.ospf_tab, text='OSPF Config')
        ...
        # Add Whitelist Frame
        self.whitelist_frame = ttk.LabelFrame(self.blacklist_tab, text="IP Whitelist", padding="10")
        self.whitelist_frame.pack(fill='both', expand=True, padx=10, pady=5)

        # Whitelist listbox with scrollbar
        whitelist_list_frame = ttk.Frame(self.whitelist_frame)
        whitelist_list_frame.pack(fill='both', expand=True, padx=5, pady=5)

        self.whitelist_listbox = tk.Listbox(whitelist_list_frame, height=10)
        whitelist_scrollbar = ttk.Scrollbar(whitelist_list_frame, orient="vertical", command=self.whitelist_listbox.yview)
        ...
        # Whitelist input frame
        whitelist_input_frame = ttk.Frame(self.whitelist_frame)
        whitelist_input_frame.pack(fill='x', padx=5, pady=5)

        ttk.Label(whitelist_input_frame, text="IP Address:").pack(side='left', padx=5)
        self.whitelist_ip_entry = ttk.Entry(whitelist_input_frame)
        self.whitelist_ip_entry.pack(side='left', padx=5)

        # Whitelist buttons frame
        whitelist_button_frame = ttk.Frame(self.whitelist_frame)
        whitelist_button_frame.pack(fill='x', padx=5, pady=5)

        ttk.Button(whitelist_button_frame, text="Add to Whitelist", command=self.add_to_whitelist).pack(side='left', padx=5)
        ttk.Button(whitelist_button_frame, text="Remove Selected", command=self.remove_from_whitelist).pack(side='left', padx=5)
        ttk.Button(whitelist_button_frame, text="Refresh Whitelist", command=self.refresh_whitelist).pack(side='left', padx=5)
        ...
        # Create OSPF Frame
        self.ospf_frame = ttk.LabelFrame(self.ospf_tab, text="OSPF Configuration", padding="10")
        self.ospf_frame.pack(fill='both', expand=True, padx=10, pady=5)

        # Add OSPF ID Field
        ttk.Label(self.ospf_frame, text="OSPF ID:").grid(row=0, column=0, sticky="w")
        self.ospf_id_entry = ttk.Entry(self.ospf_frame)
        self.ospf_id_entry.grid(row=0, column=1, padx=5, pady=2)

        # Adjust other OSPF fields' row numbers
        ttk.Label(self.ospf_frame, text="Router ID:").grid(row=1, column=0, sticky="w")
        self.router_id_entry = ttk.Entry(self.ospf_frame)
        self.router_id_entry.grid(row=1, column=1, padx=5, pady=2)

        ttk.Label(self.ospf_frame, text="Area ID:").grid(row=2, column=0, sticky="w")
        self.area_id_entry = ttk.Entry(self.ospf_frame)
        self.area_id_entry.grid(row=2, column=1, padx=5, pady=2)

        ttk.Label(self.ospf_frame, text="Network:").grid(row=3, column=0, sticky="w")
        self.ospf_network_entry = ttk.Entry(self.ospf_frame)
        self.ospf_network_entry.grid(row=3, column=1, padx=5, pady=2)

        ttk.Label(self.ospf_frame, text="Wildcard Mask:").grid(row=4, column=0, sticky="w")
        self.ospf_wildcard_entry = ttk.Entry(self.ospf_frame)
        self.ospf_wildcard_entry.grid(row=4, column=1, padx=5, pady=2)

        # Apply OSPF Configuration Button
        self.ospf_apply_btn = ttk.Button(self.ospf_frame, text="Apply OSPF Configuration", command=self.apply_ospf_config)
        self.ospf_apply_btn.grid(row=5, column=0, columnspan=2, pady=5)
    ...
    def refresh_dhcp_pools(self):
        """Refresh the list of DHCP pools from the router with proper parsing"""
        if self.ssh_connection:
            try:
                ...
                for line in output.splitlines():
                    if line.startswith("Pool"):
                        ...
                        self.pool_listbox.insert(tk.END, pool_name)
    ...
    def create_dhcp_pool(self):
        """Create or update a DHCP pool with validation"""
        if not self.ssh_connection:
            return

        try:
            pool_name = self.pool_name_entry.get().strip()
            if not pool_name or " " in pool_name:
                raise ValueError("Pool name cannot be empty or contain spaces")

            # Validate inputs
            if not all([pool_name, self.network_entry.get(), self.dhcp_mask_entry.get(), self.gateway_entry.get()]):
    ...
    def add_to_blacklist(self):
        """Add IP address to blacklist with logging"""
        ...
        try:
            ...
            output = self.ssh_connection.send_config_set(commands)
            self.blacklist_status.insert(tk.END, f"Adding {ip} to blacklist with logging...\n{output}\n")
    ...
    def add_to_whitelist(self):
        ...
        try:
            # Configure ACL for whitelist
            commands = [
                'conf t',
                'ip access-list extended WHITELIST',
                f'permit ip host {ip} any',
                'end'
            ]

            output = self.ssh_connection.send_config_set(commands)
            self.blacklist_status.insert(tk.END, f"Adding {ip} to whitelist...\n{output}\n")
            self.blacklist_status.see(tk.END)
            ...
            messagebox.showerror("Error", f"Failed to add IP to whitelist: {str(e)}")
    ...
    def remove_from_whitelist(self):
        ...
        ip = self.whitelist_listbox.get(selection[0]).split()[0]

        try:
            commands = [
                'conf t',
                'ip access-list extended WHITELIST',
                f'no permit ip host {ip} any',
                'end'
            ]

            output = self.ssh_connection.send_config_set(commands)
            self.blacklist_status.insert(tk.END, f"Removing {ip} from whitelist...\n{output}\n")
            self.blacklist_status.see(tk.END)

            self.refresh_whitelist()

        except Exception as e:
            messagebox.showerror("Error", f"Failed to remove IP from whitelist: {str(e)}")
    ...
    def refresh_whitelist(self):
        ...
        try:
            output = self.ssh_connection.send_command('show access-list WHITELIST')
            self.whitelist_listbox.delete(0, tk.END)

            for line in output.split('\n'):
                if 'permit ip host' in line:
                    ip = line.split('host')[1].split()[0]
                    self.whitelist_listbox.insert(tk.END, f"{ip}")

            self.blacklist_status.insert(tk.END, "Whitelist refreshed\n")
            self.blacklist_status.see(tk.END)
    ...
    def apply_ospf_config(self):
        """Apply OSPF configuration with OSPF ID"""
        if not self.ssh_connection:
            messagebox.showerror("Error", "Not connected to router")
            return

        ospf_id = self.ospf_id_entry.get().strip()
        router_id = self.router_id_entry.get().strip()
        area_id = self.area_id_entry.get().strip()
        network = self.ospf_network_entry.get().strip()
        wildcard = self.ospf_wildcard_entry.get().strip()

        if not all([ospf_id, router_id, area_id, network, wildcard]):
            messagebox.showerror("Error", "All OSPF fields are required")
            return
        ...
        try:
            ...
            output = self.ssh_connection.send_config_set(commands)
            messagebox.showinfo("Success", "OSPF configuration applied successfully")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to apply OSPF configuration: {str(e)}")
